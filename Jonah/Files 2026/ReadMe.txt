...
RV_and_Vsini_Code_combined-nres.ipynb
    Same format as RV_and_Vsini_Code_combined, for the samples from nres (seperate from the Coude sample). Same requirements, different save folders. This has NOT been updated with the final asymmetric/blended peak fit, so will have to make the proper changes in the future when using the nres data.
    Requires:
        Fit_SB1.py
        Fit_SB2.py
        Saphires
...
---------------------------------------
---------------------------------------
RV_and_Vsini_Code_Final.ipynb
    USE THIS TO FIT BROADENING FUNCTIONS. Same format as RV_and_Vsini_Code_combined, but with double-peak functionality, specifically for the asymmetric broadening function fitting. Better fitting parameters. Use this for all measurements (instead of RV_and_Vsini_Code_combined.ipynb).
    Requires:
        Fit_SB1_asym.py
        Fit_SB2_asym.py
        Saphires
...
PNGs, starstxt, and summary folders (if asymmetric, must MANUALLY input the '_asym' part to save to a different folder)
    Contains the RV plots (.png) and all extracted data (.txt) for each star.
---------------------------------------
---------------------------------------
...
---------------------------------------
SamplingRVs_v2.py
    This is an improvement on SamplingRVs.py, with more plotting options and optimization for a the different type of plotting. Used for the CDF plotting in the paper.
    Requires:
        Leiner 2022 table (.dat file, included in this folder as table1)
        RV_and_Vsini_Code data (Included in the file, though it might change with further star observation.)
---------------------------------------
---------------------------------------
Run_CDF_Plot_v1.ipynb
...
---------------------------------------
---------------------------------------
Run_CDF_Plot_v2.ipynb
    This is an improvement on Run_CDF_Plot_v2.ipynb, with a lot more plotting options (overplotting of certain stars, labelling, etc all within the same .ipynb). This is used to generate the CDF figures in the paper alongside SamplingRVs_v2.py.
    Requires
        SamplingRVs_v2.py file
            Leiner 2022 table (.dat file, included in this folder as table1)
            RV_and_Vsini_Code data/array (Included in the SamplingRVs.py file.)
---------------------------------------
---------------------------------------
Run_CDF_Mult_Plot.ipynb
    ...
    We end up not using these statistics in the final paper code.
    Requires
        SamplingRVs.py file
            Leiner 2022 table (.dat file, included in this folder as table1)
            RV_and_Vsini_Code data/array (Included in the SamplingRVs.py file.)
...
---------------------------------------
data.ipynb
    This file is is used to verify certain values of the data array used in the CDF plotters. I used this as a staging ground every time I changed the data array to ensure I didn't lose values between changes, and so that I had a concrete place to assign numbers to our data.
---------------------------------------
---------------------------------------
