...
def Run_SB2(tar,tar_spec, vel,bf,bf_sterr,bf_wstd, filename, star, date_title, BVCORR,  p0):
  ...
  # -------------------------------------------------
  # RV double-Gaussian fit (for visualization)
  # -------------------------------------------------
  # Fit d_gaussian_off to the FINAL combined BF (vel, bf),
  # seeded using the bootstrap medians so it locks onto the right peaks.
  p0_rv = np.array(p0, dtype=float).copy()
  p0_rv[1] = rv_boot_med_primary    # x01
  p0_rv[4] = rv_boot_med_secondary  # x02

  fit_g, cov_g = curve_fit(
      saph.utils.d_gaussian_off,
      vel,
      bf,
      p0=p0_rv,
      maxfev=10000
  ...
  bf_gauss_model = saph.utils.d_gaussian_off(vel, *fit_g)

  # individual Gaussian components if needed
  A1g, x01g, sig1g, A2g, x02g, sig2g, og = fit_g
  g1 = A1g * np.exp(-(vel - x01g)**2 / (2.0 * sig1g**2)) + og
  g2 = A2g * np.exp(-(vel - x02g)**2 / (2.0 * sig2g**2)) + og
  ...
  # NEW: RV Gaussian model overlay (and optional components)
  ax.plot(vel, bf_gauss_model, '-.', lw=3, label="RV double-Gaussian fit")
  ax.plot(vel, g1, '--', lw=1.5, color='red',   alpha=0.65)#, label="Gaussian component 1")
  ax.plot(vel, g2, '--', lw=1.5, color='green', alpha=0.65)#, label="Gaussian component 2")

  # Existing: rotational broadening fit overlay
